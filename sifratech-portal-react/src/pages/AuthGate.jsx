...
export default function AuthGate() {
  ...
  const handleResetPassword = async () => {
    ...
    const { success, error: resetErr } = await resetPassword(username);
    if (success) {
      setMessage('Password reset link sent to your email.');
    } else {
      setError(resetErr || 'Failed to send reset link.');
    }
  };
  ...
  return (
    <div id="authGate" style={{ display: 'flex' }}>
      <div className="auth-box">
        ...
        {isResetMode ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            <button 
              ...
              style={{ background: 'transparent', border: 'none', color: '#1A9FCC', cursor: 'pointer', fontSize: '14px', marginTop: '10px' }}
            >
              Back to Sign in
            </button>
          </div>
        ...
        {message && <div style={{ color: '#4caf50', marginTop: '15px', fontSize: '14px', textAlign: 'center' }}>{message}</div>}
        
        <div className="auth-copy" style={{ marginTop: '20px' }}>© 2025 Sifratech Technology Pvt Ltd · All rights reserved · Proprietary & Confidential</div>
      </div>
    </div>
  );
}
